...
@section('title', 'Edit Wilayah Tugas')

@section('page-title', 'Edit Wilayah Tugas')
...
@section('content')
    <div class="card">
        <form method="POST" action="{{ route('wilayahTugas.update', $wilayahTugas->no) }}">
            @csrf
            @method('PUT')
            <div class="card-body row g-3">
                    ...
                    <select name="id_kec" id="kecamatan" class="form-select" required>
                        ...
                        @foreach ($kecamatan as $kec)
                            <option value="{{ $kec->id_kec }}"
                                {{ old('id_kec', $wilayahTugas->id_kec) == $kec->id_kec ? 'selected' : '' }}>
                                {{ $kec->nama_kec }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select name="id_desa" id="desa" class="form-select" required>
                        ...
                        @foreach ($desa as $ds)
                            <option value="{{ $ds->id_desa }}"
                                {{ old('id_desa', $wilayahTugas->id_desa) == $ds->id_desa ? 'selected' : '' }}>
                                {{ $ds->nama_desa }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="id_bs" class="form-control"
                        value="{{ old('id_bs', $wilayahTugas->id_bs) }}" required>
                    @error('id_bs')
                        <div class="text-danger small">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="id_nks" class="form-control"
                        value="{{ old('id_nks', $wilayahTugas->id_nks) }}">
                    @error('id_nks')
                        <div class="text-danger small">{{ $message }}</div>
                    @enderror
                </div>

                <!-- ✅ PERBAIKAN: Dropdown Pengawas -->
                <div class="col-md-4">
                    ...
                    <select name="id_user" class="form-select" required>
                        ...
                        @foreach ($pengawas as $user)
                            ...
                            <option value="{{ $user->id }}"
                                {{ old('id_user', $wilayahTugas->id_user) == $user->id ? 'selected' : '' }}>
                                {{ $user->name }}
                            </option>
                        @endforeach
                    </select>
